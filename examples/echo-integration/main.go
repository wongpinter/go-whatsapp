...
func main() {
	...
	// Example 1: Using existing Echo instance
	log.Println("=== Example 1: Integration with Existing Echo Instance ===")
	existingEchoExample(&logger, appSecret, verifyToken)

	// Example 2: Creating new Echo server via factory
	log.Println("\n=== Example 2: Creating Echo Server via Factory ===")
	factoryEchoExample(&logger, appSecret, verifyToken)
...
func existingEchoExample(logger *zerolog.Logger, appSecret, verifyToken string) {
	...
	log.Printf("Echo server configured with webhook routes")
...
func factoryEchoExample(logger *zerolog.Logger, appSecret, verifyToken string) {
	// Create server via factory (will create new Echo instance)
	factory := httpserver.NewServerFactory()
	server, err := factory.CreateServer(
		httpserver.FrameworkEcho,
	...
	log.Printf("Factory-created Echo server configured")
}
